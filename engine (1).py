...
def _sample_poisson(lam: float) -> int:
    """Muestreo de Poisson (algoritmo de Knuth), sin depender de numpy."""
    import random
...
def monte_carlo_distribution(exp_home: float, exp_away: float, n_sims: int = 15000, max_goals: int = 7):
    """Simulación real de Monte Carlo: en vez de calcular la fórmula exacta,
    juega el partido n_sims veces al azar (con el mismo modelo de goles
    esperados + ajuste Dixon-Coles) y cuenta qué pasó. Con suficientes
    simulaciones, converge a lo mismo que el cálculo analítico -- la
    diferencia es que esto entrega la distribución COMPLETA de marcadores,
    no solo los 3 más probables."""
    counts = {}
    total_weight = 0.0
    for _ in range(n_sims):
        h = min(_sample_poisson(exp_home), max_goals)
        a = min(_sample_poisson(exp_away), max_goals)
        w = dc_tau(h, a, exp_home, exp_away, RHO) if h <= 1 and a <= 1 else 1.0
        w = max(w, 0)
        key = (h, a)
    ...
    if total_weight == 0:
        total_weight = 1.0
    grid = [{"h": h, "a": a, "p": c / total_weight} for (h, a), c in counts.items()]
    grid.sort(key=lambda r: -r["p"])
    return grid


XG_BLEND_WEIGHT = 0.35  # cuánto peso le damos al xG frente al gol real, cuando hay xG disponible
...
def train_on_match(state: dict, m: dict) -> dict:
    ...
    # El resultado real (quién ganó) SIEMPRE se basa en el marcador real, nunca en xG —
    # el xG solo afina qué tan bien venía jugando cada equipo, no reescribe quién ganó.
    if m["hs"] > m["as"]:
        actual_home = 1.0
    elif m["hs"] < m["as"]:
        actual_home = 0.0
    ...
    # Señal de ataque/defensa: si hay xG real de la API, se mezcla con el gol real.
    # Si no hay xG (pasa seguido en fútbol de selecciones), se usa solo el gol, como antes.
    xg_home = m.get("xg_home")
    xg_away = m.get("xg_away")
    goals_home = m["hs"] if xg_home is None else (1 - XG_BLEND_WEIGHT) * m["hs"] + XG_BLEND_WEIGHT * xg_home
    goals_away = m["as"] if xg_away is None else (1 - XG_BLEND_WEIGHT) * m["as"] + XG_BLEND_WEIGHT * xg_away

    eg_home, eg_away = expected_goals(state, m["home"], m["away"])
    err_h = goals_home - eg_home
    err_a = goals_away - eg_away
...
def build_predictions(state: dict):
    ...
    for m in state["matches"]:
        ...
        mc_grid = monte_carlo_distribution(eg_home, eg_away, n_sims=8000)
        out.append({
            ...
            "monte_carlo_top": [{"h": r["h"], "a": r["a"], "p": round(r["p"], 4)} for r in mc_grid[:6]],
            "outcomes": {k: round(v, 4) for k, v in outcomes.items()},
            "expected_goals": {"home": round(eg_home, 2), "away": round(eg_away, 2)},
            "confidence": conf,
            "lineups_confirmed": m.get("lineups_confirmed", False),
            "xg_used_last_train": m.get("xg_home_used", False),
        })
    return out
